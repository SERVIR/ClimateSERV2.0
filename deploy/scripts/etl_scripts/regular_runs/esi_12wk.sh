...
# Description:
# # Script to update the THREDDS server with the ESI 12week product data on a regular basis.
#
# Data Range:
...
# Dataset ID and Path
DATASET_ID="J8VRiTJF3X7XG7hRvw9H"   # PROD
cd /cserv2/django_app/ClimateSERV-2.0-Server
#DATASET_ID="GPY2FERp3K7vmRKK6CAp"   # DEV
...
# Cron Setup
# Weekly (2:18pm on Tuesdays)
# # 18 14 * * 2 /usr/bin/sh /cserv2/django_app/ClimateSERV-2.0-Server/deploy/scripts/etl_scripts/regular_runs/esi_12wk.sh >> /cserv2/cron_logs/$(date +\%Y_\%m_\%d_\%H\%M_\%S)etl__regular_runs__esi_12wk.log 2>&1
